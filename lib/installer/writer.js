...
export class Writer {
  ...
  installSkill(agentId, skillsDir, { force = false } = {}) {
    ...
    // Agentes renomeados: instalar o nome novo apaga a pasta do antigo,
    // senão os dois comandos ficam vivos lado a lado.
    const RENAMED = { 'mira-post-it': 'mira-postit' };
    const oldId = RENAMED[agentId];
    if (oldId) rmSync(join(this.projectRoot, skillsDir, oldId), { recursive: true, force: true });
  ...
  installTemplates() {
    ...
    for (const sub of ['themes', 'slides', 'decks', 'vendor', 'authoring', 'remote', 'studio', 'history']) {
