...
export default async function newDeck(args) {
  ...
  //    [Albatroz] O albatroz-deco também é theme-agnóstico: a paleta Art Déco
  //    (esmeralda + bronze) vive no bloco @MIRA:THEME dele, e injetar tema de
  //    card por cima quebraria a capa com a logo.
  const animationOnly = deck === 'sandeco-just-animation-template' || deck === 'albatroz-deco';
  if (!animationOnly) {
