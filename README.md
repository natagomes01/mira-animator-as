...
A story as a full animation: `mira-history` takes the text of a story (typically for children) and delivers a deck that plays it end to end: continuous scenes with camera moves, dissolve cuts to close-ups, weather and palette that change with the emotion, characters as open-licence SVG sprites (a bundled catalogue plus web search), storybook captions narrated by edge-tts one sentence at a time, background music with ducking, a cover with a QR code and double-click launchers that serve the deck on the local network so a phone or tablet can play it full screen. The model writes only data (`mira/historia.js`, a closed vocabulary of actors, places, scenes, actions and captions) and the versioned runtime `mira-history.js` does the rest (continuity between scenes, rewind, advance lock with a green button, pause, auto-play, light mode for mobile). Scripts scaffold the deck, install and normalise SVG actors, validate the story file, generate the narration and render per-scene screenshots for review. On request the same skill also turns the deck into a single .mp4 with narration and music, no controls on screen, at the story's own pace (with a lighter WhatsApp copy), and writes a Suno prompt for an instrumental background track that fits the story. The skill ships only the syntax skeleton, never a finished story, so every story is directed from its own text.
